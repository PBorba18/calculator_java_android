...
public class Calculator {
    ...
    private static double calculateRPN(@NonNull String rpn) {
        ...
        for (int i = 0; i < rpn.length(); i++) {
            ...
            if (getPriority(rpn.charAt(i)) == DIGIT_DOT) {
                ...
                switch (rpn.charAt(i)) {
                    ...
                    default: {
                        try {
                            throw new UnexpectedBehaviorException("Símbolo inesperado na expressão");
                        }
        ...
        try {
            if (calculationBuf.size() != 1)
                throw new UnexpectedBehaviorException("Como resultado dos cálculos, apenas um número deve permanecer");
        }
        catch (UnexpectedBehaviorException e) {
            e.printStackTrace();
            System.exit(-10);
    ...
    private static Priority getPriority(final char symbol) {
        ...
        try {
            throw new IncorrectUseException("Usando getPriority() de forma inadequada");
        }
